...
func (r *SyntheticsRepo) GetIpInfo() []V202101beta1IpInfo {
	// no lock needed as this collection is read-only
	copyItems := make([]V202101beta1IpInfo, len(r.ipInfo), len(r.ipInfo))
	copy(copyItems, r.ipInfo)
	return copyItems
}
...
func (r *SyntheticsRepo) load() {
	...
	r.ipInfo = storage.IpInfo
	r.traceRoutes = storage.TraceRoutes

	log.Printf("SyntheticsRepo successfuly loaded from %q, num tests: %d, num agents: %d, num health: %d, num ipInfo: %d, num traceRoutes: %d",
		r.fileName, len(r.tests), len(r.agents), len(r.health), len(r.ipInfo), len(r.traceRoutes))
}
