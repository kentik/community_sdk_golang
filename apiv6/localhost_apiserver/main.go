...
func main() {
	...
	router := makeRouter(cloudexportstub.NewCloudExportRepo(cloudexportFilePath), syntheticsstub.NewSyntheticsRepo(syntheticsFilePath))
...
func readAddressStoragePaths() (address string, cloudexport string, synthetics string) {
	...
	flag.StringVar(&cloudexport, "cloudexport", "CloudExportStorage.json", "JSON file path for the server to read and write the cloud export data to")
	flag.StringVar(&synthetics, "synthetics", "SyntheticsStorage.json", "JSON file path for the server to read and write the synthetics data to")
	flag.Parse()
	return
}
